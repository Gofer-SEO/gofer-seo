 ...
 * Input - Radio.
 *
 * Used to display a single radio.
 *
 * @package Gofer SEO
 * @since 1.0.0
 *
...
<?php foreach ( $items as $item_slug => $item ) :
	$input_id = $id . '-' . $item_slug;
	$checked  = ( $value === $item_slug ) ? 'checked' : '';
	?>
	<div class="gofer-seo-radio-item">
		<input
			type="radio"
		...
		>
		<label for="<?php echo esc_attr( $input_id ); ?>"><?php echo esc_html( wp_strip_all_tags( $item ) ); ?></label>
	</div>
<?php endforeach; ?>
