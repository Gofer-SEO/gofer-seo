<?php
/**
 * Admin Screen: Post Editor - Sitemap
 *
 * @package Gofer SEO
 * @since   1.0.0
 */
...
/**
 * Class Gofer_SEO_Screen_Post_Editor_Sitemap
 *
 * @since 1.0.0
 */
class Gofer_SEO_Screen_Post_Editor_Sitemap extends Gofer_SEO_Screen_Post_Editor {
	...
	public function get_active_post_types() {
		...
		$active_post_types = array_filter( $gofer_seo_options->options['modules']['sitemap']['enable_post_types'] );
	...
	public function get_input_typesets( $input_typesets ) {
		...
		if (
				isset( $gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type ]['frequency'] ) &&
				'default' !== $gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type ]['frequency']
		) {
			$default_frequency_value = $gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type ]['frequency'];
			$default_frequency_value = ' (' . $default_frequency_value . ')';
		} elseif (
				isset( $gofer_seo_options->options['modules']['sitemap']['post_type_default_frequency'] )
		) {
			$default_frequency_value = $gofer_seo_options->options['modules']['sitemap']['post_type_default_frequency'];
			$default_frequency_value = ' (' . $default_frequency_value . ')';
		}
		...
		if ( isset( $input_typesets['gofer_seo_modules']['wrap']['sitemap'] ) ) {
			$input_typesets['gofer_seo_modules']['wrap']['sitemap'] = array_replace( $input_typesets['gofer_seo_modules']['wrap']['sitemap'], $new_typeset );
		} else {
			$input_typesets['gofer_seo_modules']['wrap']['sitemap'] = $new_typeset;
		}
	...
	public function get_values( $values ) {
		...
		if ( ! isset( $values['gofer_seo_modules']['sitemap'] ) ) {
			$values['gofer_seo_modules']['sitemap'] = array();
		}

		$values['gofer_seo_modules']['sitemap'] = array_replace( $values['gofer_seo_modules']['sitemap'], $gofer_seo_post->meta['modules']['sitemap'] );
	...
	public function update_values( $values ) {
		...
		$gofer_seo_post->meta['modules']['sitemap'] = array_replace_recursive(
			$gofer_seo_post->meta['modules']['sitemap'],
