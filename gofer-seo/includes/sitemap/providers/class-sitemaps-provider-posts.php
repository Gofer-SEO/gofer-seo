<?php
/**
 * Gofer SEO Sitemaps: Provider - Posts
 *
...
/**
 * Class Gofer_SEO_Sitemaps_Provider_Posts.
 *
 * @since 1.0.0
 */
class Gofer_SEO_Sitemaps_Provider_Posts extends Gofer_SEO_Sitemaps_Provider {
	...
	/**
	 * Gofer_SEO_Sitemaps_Provider_Posts constructor.
	 *
	 * @since 1.0.0
	 */
	public function __construct() {
		...
		add_filter( 'gofer_seo_sitemap_post_types', array( $this, 'sitemap_post_types' ) );

		add_filter( 'wp_sitemaps_posts_query_args', array( $this, 'sitemaps_posts_query_args' ), 10, 2 );
		add_filter( 'gofer_seo_sitemaps_posts_query_args', array( $this, 'sitemaps_posts_query_args' ), 10, 2 );

		add_filter( 'wp_sitemaps_posts_entry', array( $this, 'sitemaps_posts_entry' ), 10, 3 );
		add_filter( 'gofer_seo_sitemaps_posts_entry', array( $this, 'sitemaps_posts_entry' ), 10, 3 );
	}
	...
	public function get_object_subtypes() {
		 ...
		 */
		return apply_filters( 'gofer_seo_sitemap_post_types', $post_types );
	}
...
//	}
	...
	public function get_url_list( $page_num, $post_type = '' ) {
		...
		foreach ( $query->posts as $post ) {
			 ...
			 */
			$sitemap_entry = apply_filters( 'gofer_seo_sitemaps_posts_entry', $sitemap_entry, $post, $post->post_type );
	...
	public function sitemaps_posts_entry( $sitemap_entry, $post, $post_type ) {
		...
		// Priority.
		if ( -1 !== $gofer_seo_post->meta['modules']['sitemap']['priority'] ) {
			$sitemap_entry['priority'] = $gofer_seo_post->meta['modules']['sitemap']['priority'];
		} elseif ( -1 !== $gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type ]['priority'] ) {
			$sitemap_entry['priority'] = $gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type ]['priority'];
		} elseif ( -1 !== $gofer_seo_options->options['modules']['sitemap']['post_type_default_priority'] ) {
			$sitemap_entry['priority'] = $gofer_seo_options->options['modules']['sitemap']['post_type_default_priority'];
		...
		// Frequency.
		if ( 'default' !== $gofer_seo_post->meta['modules']['sitemap']['frequency'] ) {
			$sitemap_entry['changefreq'] = $gofer_seo_post->meta['modules']['sitemap']['frequency'];
		} elseif ( 'default' !== $gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type ]['frequency'] ) {
			$sitemap_entry['changefreq'] = $gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type ]['frequency'];
		} elseif ( 'default' !== $gofer_seo_options->options['modules']['sitemap']['post_type_default_frequency'] ) {
			$sitemap_entry['changefreq'] = $gofer_seo_options->options['modules']['sitemap']['post_type_default_frequency'];
		...
		// Images.
		$images = $gofer_seo_post->get_images();
