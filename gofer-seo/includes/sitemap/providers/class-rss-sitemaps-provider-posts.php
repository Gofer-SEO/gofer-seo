<?php
/**
 * Gofer SEO (RSS) Sitemaps: Provider - Posts
 *
...
/**
 * Class Gofer_SEO_RSS_Sitemaps_Provider_Posts.
 *
 * @since 1.0.0
 */
class Gofer_SEO_RSS_Sitemaps_Provider_Posts extends Gofer_SEO_RSS_Sitemaps_Provider {
	...
	/**
	 * Gofer_SEO_RSS_Sitemaps_Provider_Posts constructor.
	 *
	 * @since 1.0.0
	 */
...
//		add_filter( 'wp_sitemap_post_types', array( $this, 'sitemap_post_types' ) );
		add_filter( 'gofer_seo_rss_sitemap_post_types', array( $this, 'sitemap_post_types' ) );

//		add_filter( 'wp_sitemaps_posts_query_args', array( $this, 'sitemaps_posts_query_args' ), 10, 2 );
		add_filter( 'gofer_seo_rss_sitemaps_posts_query_args', array( $this, 'sitemaps_posts_query_args' ), 10, 2 );

//		add_filter( 'wp_sitemaps_posts_entry', array( $this, 'sitemaps_posts_entry' ), 10, 3 );
		add_filter( 'gofer_seo_rss_sitemaps_posts_entry', array( $this, 'sitemaps_posts_entry' ), 10, 3 );
	}
	...
	public function get_object_subtypes() {
		 ...
		 */
		return apply_filters( 'gofer_seo_rss_sitemap_post_types', $post_types );
	}
	...
	public function get_query_args( $post_types ) {
		 ...
		 */
		return apply_filters( 'gofer_seo_rss_sitemaps_posts_query_args', $args, $post_types );
	}
...
//	}
	...
	public function get_url_list( $page_num, $post_type = '' ) {
		...
		foreach ( $query->posts as $post ) {
			 ...
			 */
			$sitemap_entry = apply_filters( 'gofer_seo_rss_sitemaps_posts_entry', $sitemap_entry, $post, $post->post_type );
	...
	public function sitemap_post_types( $post_types ) {
		...
		foreach ( $enabled_post_types as $post_type_name ) {
			if (
					...
					$gofer_seo_options->options['modules']['sitemap']['post_type_settings'][ $post_type_name ]['show_on']['rss']
			) {
	...
	public function sitemaps_index_entry( $sitemap_entry, $object_type, $object_subtype, $page ) {
		...
		if ( $query->have_posts() ) {
			...
			if ( $timestamp ) {
				$sitemap_entry['lastmod'] = mysql2date( 'r', $timestamp );
			}
		}
	...
	public function sitemaps_posts_entry( $sitemap_entry, $post, $post_type ) {
		...
		$pub_date = '';
		if ( $timestamp ) {
			$pub_date = mysql2date( 'r', $timestamp );
		}

		$rss_sitemap_entry = array(
			'title'       => get_the_title( $post ),
			//'link'        => $sitemap_entry['link'],
			'guid'        => get_the_guid( $post ),
			'pubDate'     => $pub_date,
			'description' => get_post_field( 'post_excerpt', $post->ID ),
		);

		$sitemap_entry = wp_parse_args( $rss_sitemap_entry, $sitemap_entry );
		return $sitemap_entry;
	}
