...
/**
 * Class Gofer_SEO_Graph_Organization.
 *
 * @since 1.0.0
 *
 ...
 */
class Gofer_SEO_Graph_Organization extends Gofer_SEO_Graph {
	...
	protected function prepare() {
		...
		if ( 'person' === $gofer_seo_options->options['modules']['schema_graph']['site_represents'] ) {
			$person_id = intval( $gofer_seo_options->options['modules']['schema_graph']['person_user_id'] );
			...
			if ( -1 === $person_id ) {
				...
				$rtn_data['name'] = $gofer_seo_options->options['modules']['schema_graph']['person_custom_name'];

				// Handle Logo/Image.
				$image_data   = wp_parse_args( array( 'url' => $gofer_seo_options->options['modules']['schema_graph']['organization_logo'] ), $this->get_image_data_defaults() );
			...
			if ( $gofer_seo_options->options['modules']['schema_graph']['organization_name'] ) {
				$rtn_data['name'] = $gofer_seo_options->options['modules']['schema_graph']['organization_name'];
			} else {
			...
			// Handle contactPoint.
			if ( ! empty( $gofer_seo_options->options['modules']['schema_graph']['phone_number'] ) ) {
	...
	protected function prepare_contactpoint() {
		...
		$rtn_data = array(
			'@type'       => 'ContactPoint',
			'telephone'   => '+' . $gofer_seo_options->options['modules']['schema_graph']['phone_number'],
			'contactType' => $gofer_seo_options->options['modules']['schema_graph']['phone_contact_type'],
		);
	...
	protected function get_site_social_profile_links() {
		...
		if ( ! empty( $gofer_seo_options->options['modules']['schema_graph']['social_profile_urls'] ) ) {
			$social_links = $gofer_seo_options->options['modules']['schema_graph']['social_profile_urls'];
	...
	protected function get_logo_id() {
		...
		if ( ! empty( $gofer_seo_options->options['modules']['schema_graph']['organization_logo'] ) ) {
			// Changes the URL to an ID. Known to be memory intense.
			// Option configurations need to use IDs rather than the URL strings.
			$logo_id = Gofer_SEO_Methods::attachment_url_to_postid( $gofer_seo_options->options['modules']['schema_graph']['organization_logo'] );
		}
