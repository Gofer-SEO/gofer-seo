 ...
 */
class Gofer_SEO_Graph_Article extends Gofer_SEO_Graph_CreativeWork {
	...
	protected function prepare() {
		...
		$rtn_data = array(
			...
			'publisher'        => array( '@id' => home_url() . '/#' . $gofer_seo_options->options['modules']['schema_graph']['site_represents'] ),
	...
	protected function get_article_image_data( $post ) {
		...
		} elseif ( 'organization' === $gofer_seo_options->options['modules']['schema_graph']['site_represents'] ) {
			if ( ! empty( $gofer_seo_options->options['modules']['schema_graph']['organization_logo'] ) ) {
				$rtn_image_data = $this->get_site_image_data( $gofer_seo_options->options['modules']['schema_graph']['organization_logo'] );
			}
		} elseif ( 'person' === $gofer_seo_options->options['modules']['schema_graph']['site_represents'] ) {
			$rtn_image_data = $this->get_site_image_data( intval( $post->post_author ) );
		} else {
